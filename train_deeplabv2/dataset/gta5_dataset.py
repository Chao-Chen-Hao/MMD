...
if __name__ == '__main__':
    dst = gta5DataSet('../../Warehouse/GTA5/', './gta5_list/train.txt', mean=(0,0,0), autoaug=True)
    trainloader = data.DataLoader(dst, batch_size=4)
    for i, data in enumerate(trainloader):
        imgs, labels, _, _ = data
        if i == 0:            
            img = torchvision.utils.make_grid(imgs[0]).numpy()
            img = np.transpose(img, (1, 2, 0))
            img = img[:, :, ::-1]
            img = Image.fromarray(np.uint8(img) )
            img.save('GTA5_Demo_img.jpg')
            
            img = Image.fromarray(np.uint8(labels[0]*100) )
