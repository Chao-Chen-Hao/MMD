...
from utils.fusion_methods import priority_aggregation, majority_aggregation, source_distribution_aggregation, certainty_aggregation
...
def get_arguments():
    ...
    #### Model Settings  ####
    parser.add_argument("--fusion-mode", type=str, default=FUSION_MODE, choices=['majority', 'certainty', 'priority'],
                        help="available options : majority, certainty, priority")
...
def main():
    ...
    if args.source_domain == 'synthia':
        targetloader_CRST = data.DataLoader(
    ...
    if args.fusion_methods == 'source_distribution':
        source_map = np.load('map.npy') # size: (512, 1024, 19)
        source_map = torch.from_numpy(source_map).cuda(args.gpu_id)
        source_map = source_map.type(torch.float)
        source_map = interp(torch.unsqueeze(source_map.permute(2,0,1), 0))
        _, source_map = torch.max(source_map, axis=1)
    
    if args.source_domain == 'synthia':
        # evaluate
        for index, img_data in enumerate(zip(targetloader_CRST, targetloader_Mobile, targetloader_MRNet) ):
            ...
            label_arr_1 = label_arr_1.cuda(args.gpu_id)#*1.6
            label_arr_2 = label_arr_2.cuda(args.gpu_id)
            
            ### ----------------
            # aggregation
            # [index, method]
            # (0:CRST) (1:Mobile) (2:MRNet)
            priority_list = [
                [4,0], 
                [3,0],
                [11,2],
            ...
            with torch.no_grad():
                if args.aggregate_mode == 'priority':
                    pseudo_label = priority_aggregation(priority_list, label_list, certainty_list, args.threshold, args.gpu_id)
                if args.aggregate_mode == 'majority':
                    pseudo_label = majority_aggregation(priority_list, label_list, certainty_list, args.threshold, args.gpu_id)
                if args.aggregate_mode == 'source-distribution':
                    pseudo_label = source_distribution_aggregation(priority_list, label_list, certainty_list, args.gpu_id, source_map[0])
                if args.aggregate_mode == 'certainty':
                    pseudo_label = certainty_aggregation(priority_list, label_list, certainty_list, args.threshold, args.gpu_id)
